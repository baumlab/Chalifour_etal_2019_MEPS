...
>3) We used AICc to rank all models in dredge, as it penalizes the strength of likelihood more for very small sample sizes, and is more adaptive to moderate sample sizes, so that we could apply the same information criterion across all models (Harrison et al 2018; Brewer et al 2016) -- could explain better...  
...
#Check model assumptions - note that several parameters have non ideal relationships. Also note that there are several outliers, but these are real so we do not remove them (despite what this test says).   
sjp.glm(res, type = "ma")  
