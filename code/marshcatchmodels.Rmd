...
>1) Decided we should NOT include site as a random effect for all beach seine models because the low # of sites and model performance was worse than GLMs without random effect for all species groups.  
...
response = average abundance of Chinook salmon in marsh sites
...
library(sjPlot)
#Check model assumptions - note that we are not assuming linear relationships with Jday
...
response = average abundance of chum salmon  
...
##For cases with Jday^2, need to use plotmath to create symbol. 
...
response = average abundance of migratory fishes
...
response = average abundance of residents
